...
import com.direwolf20.buildinggadgets.api.utils.RegistryUtil;
...
public final class BlockData {
    ...
    public static BlockData deserialize(NBTTagCompound tag, boolean persisted) {
        ...
        try {
            if (persisted)
                serializer = RegistryUtil
                        .getFromString(Registries.getTileDataSerializers(), tag.getString(KEY_SERIALIZER));
            else
                serializer = RegistryUtil.getById(Registries.getTileDataSerializers(), tag.getInt(KEY_SERIALIZER));
        } catch (Exception e) {
    ...
    public NBTTagCompound serialize(boolean persisted) {
        ...
        if (persisted)
            ...
            tag.setInt(KEY_SERIALIZER, RegistryUtil
