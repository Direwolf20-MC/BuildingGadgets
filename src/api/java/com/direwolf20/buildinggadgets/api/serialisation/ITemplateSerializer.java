...
/**
 * This class represents a Serializer responsible for converting {@link ITemplate}'s into {@link CompoundNBT}'s and back. Additionally an {@code ITemplateSerializer}
 ...
 * <p>
 * Additionally the {@link #serialize(ITemplate, boolean)} and {@link #deserialize(CompoundNBT, TemplateHeader, boolean)} methods are passed an boolean flag in order to hint
...
public interface ITemplateSerializer extends IForgeRegistryEntry<ITemplateSerializer> {
     ...
     * @return A {@link CompoundNBT} representing the serialized form of the given {@link ITemplate}. May be empty but <b>not null</b> to
     *         indicate that no data requires serialization.
     * @throws IllegalArgumentException If the {@link ITemplate} implementation is not supported by this serializer or
     *         if a persisted save is attempted to be retrieved non-persistently or vice-versa.
     */
    CompoundNBT serialize(ITemplate template, boolean persisted);

    /**
     * Deserializes an {@link ITemplate} from the given {@link CompoundNBT}.
     * @param tagCompound The {@link CompoundNBT} to deserialize an {@link ITemplate} for.
     * @param header The {@link TemplateHeader} for the corresponding {@link CompoundNBT} or null if not present.
     * @param persisted Whether or not this data was previously persisted. It can be expected that this flag matches
     *         whatever was passed into {@link #serialize(ITemplate, boolean)} to create the {@link CompoundNBT}.
     * @return The {@link ITemplate} representing the serialized data in the {@link CompoundNBT}.
     ...
     */
    ITemplate deserialize(CompoundNBT tagCompound, @Nullable TemplateHeader header, boolean persisted);
}
