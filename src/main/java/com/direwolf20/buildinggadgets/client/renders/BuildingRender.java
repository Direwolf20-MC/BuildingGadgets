...
public class BuildingRender extends AbstractRender {
    ...
    public void gadgetRender(Tessellator tessellator, BufferBuilder bufferBuilder, RayTraceResult rayTraceResult, IBlockState traceBlock, ItemStack gadget, List<BlockPos> existingLocations) {
        IBlockState renderBlockState = getToolBlock(gadget);
        ...
        List<BlockPos> locations = existingLocations.size() != 0
                ? existingLocations
                : BuildingModes.BUILD_TO_ME.getMode().getCollection(mc.player, mc.player.world, renderBlockState, rayTraceResult.getBlockPos(), mc.player.getPosition(), rayTraceResult.sideHit, GadgetUtils.getToolRange(gadget), GadgetBuilding.shouldPlaceAtop(gadget), GadgetBuilding.getFuzzy(gadget));


        // todo: work out items and energy
...
//            hasBlocks--;
//            if (gadget.hasCapability(CapabilityEnergy.ENERGY, null)) {
...
//            if (hasBlocks < 0 || hasEnergy < 0) {
                rendererDispatcher.renderBlockBrightness(Blocks.STAINED_GLASS.getDefaultState().withProperty(COLOR, EnumDyeColor.RED), 1f);
//            }
