...
public final class ExclusiveAxisChasing implements IPlacementSequence {
    ...
    public static ExclusiveAxisChasing create(BlockPos source, BlockPos target, Axis axis) {
        int difference = VectorHelper.getAxisValue(target, axis) - VectorHelper.getAxisValue(source, axis);
        if (difference < 0)
    ...
    public boolean mayContain(int x, int y, int z) {
        Axis axis = offsetDirection.getAxis();
        int value = VectorHelper.getAxisValue(x, y, z, axis);
        int sourceValue = VectorHelper.getAxisValue(source, axis);
