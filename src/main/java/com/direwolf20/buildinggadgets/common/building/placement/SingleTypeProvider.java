...
public class SingleTypeProvider implements IBlockProvider {
    ...
    public void serialize(NBTTagCompound tag) {
        tag.setTag(NBTKeys.SINGLE_TYPE_PROVIDER_TAG, NBTUtil.writeBlockState(state));
    }

    @Override
    public SingleTypeProvider deserialize(NBTTagCompound tag) {
        return new SingleTypeProvider(NBTUtil.readBlockState((NBTTagCompound) tag.getTag(NBTKeys.SINGLE_TYPE_PROVIDER_TAG)));
    }
