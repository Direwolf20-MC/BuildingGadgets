...
import com.direwolf20.buildinggadgets.network.PacketPasteGUI;
...
public class PasteGUI extends GuiScreen {
    ...
    protected void nullCheckTextBoxes() {
        if (X.getText() == "") {
            X.setText(String.valueOf(CopyPasteTool.getX(tool)));
        }
        if (Y.getText() == "") {
            Y.setText(String.valueOf(CopyPasteTool.getY(tool)));
        }
        if (Z.getText() == "") {
            Z.setText(String.valueOf(CopyPasteTool.getZ(tool)));
        }
    }
